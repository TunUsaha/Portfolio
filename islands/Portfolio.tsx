...
function NavBar(
  ...
  const barStyle = (active: boolean): string =>
  ...
  const btnMagnet = (e: MouseEvent) => {
    ...
    el.style.transform = `translate(${(dx * 0.3).toFixed(1)}px,${(dy * 0.45).toFixed(1)}px)`;
  };
  const btnReset = (e: MouseEvent) => {
  ...
  return (
    <nav style='position:sticky;top:0;z-index:50;display:flex;align-items:center;justify-content:space-between;padding:0 clamp(20px,5vw,72px);height:68px;background:var(--glass);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);border-bottom:1px solid var(--hair)'>
      ...
      <div style='display:flex;align-items:center;gap:clamp(14px,3vw,40px)'>
        {(
          [['home', 'Home'], ['about', 'About'], ['experience', 'Experience'], ['projects', 'Projects']] as [Page, string][]
        ).map(([p, label]) => (
          <button
            key={p}
            onClick={() => onNav(p)}
...
function HomePage({ onNav }: { onNav: (p: Page) => void }) {
  ...
  const heroEnter = (e: MouseEvent) => {
    ...
    bands.forEach((b) => {
      const kf = b.dataset.kf!;
      const dur = b.dataset.dur!;
      b.style.animation = `${kf} ${dur} ease-in-out 1`;
      const done = () => {
        b.style.animation = 'none';
        b.removeEventListener('animationend', done);
        remaining -= 1;
        if (remaining === 0) sec.dataset.anim = '';
      };
    ...
    <main style='max-width:1320px;margin:0 auto;padding:clamp(32px,6vw,84px) clamp(20px,5vw,72px) 96px'>
      ...
      <section style='display:grid;grid-template-columns:1.25fr .9fr;gap:clamp(24px,5vw,64px);align-items:end'>
        <div>
          ...
          <h1 style='margin:0;font-weight:800;letter-spacing:-.045em;line-height:.92;color:var(--headline);font-size:clamp(56px,13vw,184px)'>
            Tun<br />Usaha
          </h1>
        </div>
      ...
      <section style='margin-top:clamp(56px,7vw,104px)'>
        <div style='display:flex;align-items:flex-end;justify-content:space-between;gap:20px;flex-wrap:wrap;margin-bottom:32px;animation:revealIn .7s cubic-bezier(.2,.7,.2,1) both'>
          ...
          <p style='margin:0;max-width:340px;font-size:14px;line-height:1.6;color:var(--muted)'>
            The questions, fields, and ideas I keep circling back to — the stuff that makes the work feel like play.
          </p>
        </div>
        <div style='display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:16px'>
          {interestingCards.map((c, i) => (
            <div
              key={c.num}
            ...
            >
              ...
              <div style='display:flex;flex-wrap:wrap;gap:8px'>
                {s.tags.map((tag) => (
                  <span
                    key={tag}
                    style='font-size:13px;color:var(--body);border:1px solid var(--border);border-radius:999px;padding:5px 12px;background:var(--bg)'
                  >
                    {tag}
                  </span>
                ))}
...
function ExperiencePage() {
  ...
  // certSrcs holds image srcs keyed by cert id — set when user drops images in future
  const certSrcs: Record<string, string> = {};
  ...
  const openCert = (id: string) => {
    const src = certSrcs[id];
    if (src) setLightboxSrc(src);
  };

  const certCards = [
    { id: 'cert-lex-1', title: 'LeX SP × CMU — Cohort 1', sub: 'Sustainable Innovation Camp · participation', placeholder: 'LeX SP × CMU certificate 01' },
    { id: 'cert-lex-2', title: 'LeX SP × CMU — Cohort 2', sub: 'Sustainable Innovation Camp · participation', placeholder: 'LeX SP × CMU certificate 02' },
    { id: 'cert-award', title: 'APacCHRIE 2025', sub: 'Award certificate', placeholder: 'APacCHRIE 2025 award image' },
  ];
      ...
      <div style='position:relative;padding-left:28px'>
        ...
        <article style='position:relative;padding:0 0 44px'>
          ...
          <p style='margin:0;max-width:660px;font-size:15px;line-height:1.65;color:var(--body)'>
            Analyzed business processes and customer requirements to bridge the gap between business needs and CRM solutions. Contributed to end-to-end CRM implementation, from system design and testing to user training and continuous support.
          </p>
        </article>
        ...
        <article style='position:relative;padding:0 0 8px'>
          ...
          <p style='margin:0;max-width:660px;font-size:15px;line-height:1.65;color:var(--body)'>
            Explored community challenges through field research and developed prototype solutions using Design Thinking and sustainable innovation approaches.
          </p>
        </article>
      </div>
      ...
      <section style='margin-top:clamp(36px,4.5vw,56px)'>
        <div style='display:flex;align-items:flex-end;justify-content:space-between;gap:20px;flex-wrap:wrap;margin-bottom:24px;padding-top:clamp(28px,4vw,40px);border-top:1px solid var(--hair);animation:revealIn .7s cubic-bezier(.2,.7,.2,1) both'>
          ...
          <p style='margin:0;max-width:380px;font-size:14px;line-height:1.6;color:var(--muted)'>
            Supporting documents behind the milestones above — participation in the LeX SP × CMU innovation camps and the APacCHRIE 2025 award. Click any certificate to view it full size.
          </p>
        </div>
            ...
            >
              ...
              >
                <div style='position:absolute;inset:12px;display:flex;align-items:center;justify-content:center;border:1px dashed var(--border);border-radius:6px'>
                  <span style='font-size:12px;color:var(--muted);text-align:center;padding:8px'>{c.placeholder}</span>
                </div>
                {/* Zoom icon */}
...
function ProjectsPage() {
  ...
  const spikeStyles = (i: number) => {
    ...
    return {
      orbitTop: {
        position: 'absolute' as const, width: W + 'px', zIndex: 0, pointerEvents: 'none' as const,
        lineHeight: '0', top: '-24px', right: '-18px',
        transform: hovered ? `translateX(${-distX}px)` : 'translateX(0px)',
        transition: 'transform .8s cubic-bezier(.22,.68,.2,1)',
      },
      orbitBot: {
        position: 'absolute' as const, width: W + 'px', zIndex: 0, pointerEvents: 'none' as const,
        lineHeight: '0', bottom: '-24px', left: '-18px',
        transform: hovered ? `translateX(${distX}px)` : 'translateX(0px)',
        transition: 'transform .8s cubic-bezier(.22,.68,.2,1)',
      },
      spinTop: {
        width: W + 'px', height: 'auto', display: 'block', opacity: 0.95,
        filter: 'drop-shadow(0 3px 7px rgba(0,0,0,.18))', transformOrigin: 'center',
...
export default function Portfolio() {
  ...
  const themeVars = (() => {
    ...
    if (theme === 'dark') {
      return { '--bg': '#161616', '--surface': '#262626', '--headline': '#f6f6f6', '--body': '#d2d2d2', '--muted': '#969696', '--hair': '#303030', '--border': '#484848', '--glass': 'rgba(22,22,22,.6)', '--accent': accent, '--hero-vignette': 'rgba(0,0,0,.04)' };
    }
    ...
    <div style={rootStyle}>
      ...
      <footer style='border-top:1px solid var(--hair);padding:34px clamp(20px,5vw,72px);display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px'>
        ...
        <div style='display:flex;align-items:center;gap:22px;flex-wrap:wrap'>
          <a href='https://github.com/TunUsaha' target='_blank' rel='noreferrer' style='font-size:13px;color:var(--body);text-decoration:none'>
            GitHub @TunUsaha ↗
          </a>
          <a href='https://www.kaggle.com/tunusaha' target='_blank' rel='noreferrer' style='font-size:13px;color:var(--body);text-decoration:none'>
            Kaggle @tunusaha ↗
          </a>
        </div>
      </footer>
